...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    const uploadPath = `uploads/RaiseRequest/${req.body.propertyID}/${
      today.getDate() + 1
  ...
  filename: (req, file, cb) => {
    const today = new Date();
    const directory = `uploads/RaiseRequest/${req.body.propertyID}/${
      today.getDate() + 1
    }-${today.getMonth()}-${today.getFullYear()}/`;
    const files = fs
      .readdirSync(directory)
...
const uploadRaiseRequestImages = multer({ storage: storage });

module.exports = uploadRaiseRequestImages;
